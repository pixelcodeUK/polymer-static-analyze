...
function exportFirebase(file,analysis){
  var map = {
    'hosting': {
      'headers':[]
    }
  ...
  for(var i = 0;i < analysis.length;i++){
    ...
    analysis[i].dependencies.forEach(function(value){
      valstring += '<'+value.file.replace(path.dirname(file),'')+'>';
      //Preload files that do not have a high reference count
      if(value.refCount < 2){
        valstring += ';rel=preload';
      }
      valstring += ';as=document,';
    });

    valstring = valstring.substring(0,valstring.length-1);

    map.hosting.headers.push({
      'source' : analysis[i].file,
      'headers': [{'key': 'Link', 'value': valstring}]
    });
  }

  return new Promise(function(resolve, reject) {
    fs.writeFile('firebase.json', JSON.stringify(map), function(err) {
      if(err){
        reject('Could not write firebase.json');
      }else{
        resolve('firebase.json saved');
      }
    });
  });
}
...
exports.exportFirebase = exportFirebase;
